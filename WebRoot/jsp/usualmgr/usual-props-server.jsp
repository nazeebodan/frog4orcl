...
function secBoard(v){
   var url;
   if(v==0){
      url = "/usualMgr.do?method=queryPropsServerInfo";
   }else if(v==1){
      url = "/usualMgr.do?method=queryPropsClientInfo"
   }else if(v==2){
      url = "/usualMgr.do?method=queryPropsSessionInfo";
   }
...
<body>
<form method="post" action="<%=path%>/usualMgr.do?method=queryPropsServerInfo">
<div id="center-column">
			...
			<TABLE width=72% border=0 cellPadding=0 cellSpacing=0 id=secTable>
                <TBODY>
                  <TR align=middle height=20>
                    <TD align="center" onclick=secBoard(0) style="background-color:#F7EFF1;">server端字符集信息</TD>
                    <TD align="center" onclick=secBoard(1)>client端字符集信息</TD>
                    <TD align="center" onclick=secBoard(2)>session字符集信息</TD>
                  </TR>
                </TBODY>
              </TABLE>
		  </div>
		  <br /><HR style="FILTER: alpha(opacity=100,finishopacity=0,style=1)" width="80%" color=#987cb9 SIZE=3>
			<b>server端字符集信息:</b><div class="table">
				<img src="<%=path %>/jsp/init/images/bg-th-left.gif" width="8" height="7" alt="" class="left" />
				<table class="listing" cellpadding="0" cellspacing="0">
					<tr>
						<c:if test="${info1.data!= null}">
							<c:forEach var="columns" items="${info1.data.columns}" varStatus="s">
								<c:if test="${s.first}">
					...
					<c:if test="${info1.data!=null}">
						<c:if test="${info1.data.data!=null}">
							<c:forEach var="rowMap" items="${info1.data.data.rows}">
							<tr>
								<c:forEach var="columns" items="${info1.data.columns}" >
		  ...
		  <br /><HR style="FILTER: alpha(opacity=100,finishopacity=0,style=1)" width="80%" color=#987cb9 SIZE=3>
			<b>server端字符集信息列表:</b><div class="table">
				<img src="<%=path %>/jsp/init/images/bg-th-left.gif" width="8" height="7" alt="" class="left" />
				<table class="listing" cellpadding="0" cellspacing="0">
					<tr>
