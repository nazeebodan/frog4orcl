...
<body>
<form method="post" action="<%=path%>/usualMgr.do?method=queryUserHaveObjectsInfo">
			...
			<TABLE width=72% border=0 cellPadding=0 cellSpacing=0 id=secTable>
                <TBODY>
                  <TR align=middle height=20>
                    ...
                    <TD align="center" onclick=secBoard(2) style="background-color:#F7EFF1;">用户所含对象</TD>
                  </TR>
                </TBODY>
              </TABLE>
		  </div>
		  <br /><HR style="FILTER: alpha(opacity=100,finishopacity=0,style=1)" width="80%" color=#987cb9 SIZE=3>
			<b>用户拥有的对象描述:</b><div class="table">
				...
				<table class="listing" cellpadding="0" cellspacing="0">
					...
					<c:if test="${userHaveObjectsInfo.data!=null}">
						<c:if test="${userHaveObjectsInfo.data.data!=null}">
							<c:forEach var="rowMap" items="${userHaveObjectsInfo.data.data.rows}">
							<tr>
								<td><c:out value="${rowMap['OWNER']}"></c:out></td>
								<td><c:out value="${rowMap['OBJECT_TYPE']}"></c:out></td>
								<td><c:out value="${rowMap['CN']}"></c:out></td>
							</tr>
							</c:forEach>
						</c:if>
					</c:if>
